-- ============================================================================
-- FEAT-GH-009-E STEP 3 (FIXED): Create Progress Tracking Tables
-- ============================================================================
-- Purpose: Create phase, lesson, and tactic progress tables
-- Run AFTER: 014c_CLEANUP_first.sql
-- ============================================================================
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'gh_user_phase_progress_unique_user_phase'
  ) THEN
    ALTER TABLE public.gh_user_phase_progress
      ADD CONSTRAINT gh_user_phase_progress_unique_user_phase UNIQUE (user_id, phase_id);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_gh_phase_progress_user_id ON public.gh_user_phase_progress(user_id);
CREATE INDEX IF NOT EXISTS idx_gh_phase_progress_phase_id ON public.gh_user_phase_progress(phase_id);
CREATE INDEX IF NOT EXISTS idx_gh_phase_progress_status ON public.gh_user_phase_progress(status);
CREATE INDEX IF NOT EXISTS idx_gh_phase_progress_user_status ON public.gh_user_phase_progress(user_id, status);
...
CREATE TABLE IF NOT EXISTS public.gh_user_lesson_progress (
  ...
  CONSTRAINT gh_user_lesson_progress_valid_video_percent CHECK (video_watched_percent BETWEEN 0 AND 100),
  CONSTRAINT gh_user_lesson_progress_valid_tactics_percent CHECK (tactics_completion_percent BETWEEN 0 AND 100)
);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'gh_user_lesson_progress_unique_user_lesson'
  ) THEN
    ALTER TABLE public.gh_user_lesson_progress
      ADD CONSTRAINT gh_user_lesson_progress_unique_user_lesson UNIQUE (user_id, lesson_id);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_gh_lesson_progress_user_id ON public.gh_user_lesson_progress(user_id);
CREATE INDEX IF NOT EXISTS idx_gh_lesson_progress_lesson_id ON public.gh_user_lesson_progress(lesson_id);
CREATE INDEX IF NOT EXISTS idx_gh_lesson_progress_status ON public.gh_user_lesson_progress(status);
CREATE INDEX IF NOT EXISTS idx_gh_lesson_progress_user_status ON public.gh_user_lesson_progress(user_id, status);
CREATE INDEX IF NOT EXISTS idx_gh_lesson_progress_stuck ON public.gh_user_lesson_progress(status, stuck_detected_at) WHERE status = 'stuck';
CREATE INDEX IF NOT EXISTS idx_gh_lesson_progress_last_activity ON public.gh_user_lesson_progress(last_activity_at DESC);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'gh_user_tactic_completions_unique_user_tactic'
  ) THEN
    ALTER TABLE public.gh_user_tactic_completions
      ADD CONSTRAINT gh_user_tactic_completions_unique_user_tactic UNIQUE (user_id, tactic_id);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_gh_tactic_completions_user_id ON public.gh_user_tactic_completions(user_id);
CREATE INDEX IF NOT EXISTS idx_gh_tactic_completions_tactic_id ON public.gh_user_tactic_completions(tactic_id);
CREATE INDEX IF NOT EXISTS idx_gh_tactic_completions_completed_at ON public.gh_user_tactic_completions(completed_at DESC);
