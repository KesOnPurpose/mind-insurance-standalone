-- =====================================================
-- FIX ASSESSMENT LOOP - Users stuck after completing assessment
-- =====================================================
-- This migration fixes the onboarding assessment loop where users
-- can't get past the assessment page after completing it.
--
-- ROOT CAUSES FIXED:
-- 1. assessment_completed_at DEFAULT NOW() - wrong, should be NULL
-- 2. Missing UNIQUE constraint on user_id for upsert to work
-- 3. No automatic user_onboarding record creation on signup
--
...
-- =====================================================
-- STEP 1: FIX DEFAULT VALUE FOR assessment_completed_at
-- =====================================================
...
-- =====================================================
-- STEP 3: FIX EXISTING RECORDS WITH WRONG TIMESTAMPS
-- =====================================================
...
BEGIN
  ...
  IF fixed_count > 0 THEN
    RAISE NOTICE '✅ Fixed % user_onboarding records with incorrect assessment_completed_at', fixed_count;
  END IF;
END $$;
...
-- =====================================================
-- STEP 4: UPDATE TRIGGER TO CREATE user_onboarding RECORD
-- =====================================================
...
BEGIN
  ...
  -- =====================================================
  -- NEW: Also create user_onboarding record for assessment tracking
  -- =====================================================
  INSERT INTO public.user_onboarding (
    user_id,
    onboarding_step,
    has_seen_welcome,
    assessment_completed_at,  -- CRITICAL: NULL until they actually complete it
    created_at,
    updated_at
  )
  VALUES (
...
-- =====================================================
-- STEP 5: BACKFILL user_onboarding for existing users
-- =====================================================
...
BEGIN
  ...
  IF backfilled_count > 0 THEN
    RAISE NOTICE '✅ Backfilled % user_onboarding records for existing users', backfilled_count;
  END IF;
END $$;
...
BEGIN
  ...
  IF users_without_onboarding = 0 AND users_with_wrong_timestamp = 0 THEN
    RAISE NOTICE '';
    RAISE NOTICE '✅ ALL CHECKS PASSED - Assessment loop should be fixed!';
  ELSE
    RAISE NOTICE '';
    RAISE NOTICE '⚠️ Some issues remain - please review';
  END IF;
