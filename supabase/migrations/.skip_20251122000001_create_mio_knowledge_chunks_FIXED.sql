-- =====================================================================================
-- MIO Knowledge Chunks Table - CORRECTED VERSION
...
DO $$
BEGIN
  ASSERT (SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'mio_knowledge_chunks'));
  ASSERT (SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'mio_knowledge_chunks' AND column_name = 'temperament_match'));
  ASSERT (SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'mio_knowledge_chunks' AND column_name = 'state_created'));
  RAISE NOTICE '==================================================';
  RAISE NOTICE 'MIO Knowledge Chunks Migration Complete!';
  RAISE NOTICE '==================================================';
  RAISE NOTICE 'Table: mio_knowledge_chunks';
  RAISE NOTICE 'Indexes: 13 specialized indexes created';
  RAISE NOTICE 'Function: search_mio_knowledge (hybrid search)';
