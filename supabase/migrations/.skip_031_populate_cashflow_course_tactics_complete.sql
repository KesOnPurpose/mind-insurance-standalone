-- Migration: Populate all cashflow course tactics (150 tactics)
-- Date: 2025-12-02
-- Purpose: Insert 150 tactics from Group Home Cash Flow Course by Lynette Wheaton
-- This is Phase 2 of hybrid migration strategy (complete atomic migration)
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'gh_tactic_instructions table does not exist';
  END IF;
END $$;

-- Check tactic_source column exists
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Check for existing tactics in T439-T590 range
DO $$
DECLARE
...
BEGIN
  ...
  WHERE tactic_id BETWEEN 'T439' AND 'T590';

  IF existing_count > 0 THEN
    RAISE NOTICE 'Found % existing tactics in T439-T590 range. Will use ON CONFLICT to handle.', existing_count;
  END IF;
END $$;
...
DROP TABLE IF EXISTS gh_tactic_instructions_backup_cashflow_phase2;
CREATE TABLE gh_tactic_instructions_backup_cashflow_phase2 AS
SELECT * FROM gh_tactic_instructions;

DO $$
BEGIN
  RAISE NOTICE '✓ Created backup table: gh_tactic_instructions_backup_cashflow_phase2';
END $$;

-- ============================================
-- SECTION 3: TACTIC INSERTION - MODULE 1
-- Business Formation & Legal (T439-T460)
-- ============================================
...
INSERT INTO gh_tactic_instructions (
  ...
  (
    ...
    '{"quote": "Make sure that you register your business, so whether it''s gonna be a LLC, so proprietorship or corporation, make sure that you register the business.", "context": "Explaining business structure options for group home operators", "source": "Group Home Cashflow Course - Business Formation Module"}'::jsonb,
    ...
    'one_time_fee',
    '["any"]'::jsonb,
    '["rental_arbitrage", "property_purchase", "partnership"]'::jsonb,
    '["any"]'::jsonb,
    'cashflow_course',
    'beginner',
    1,
  ...
  (
    ...
    '{"quote": "Before creating a name, make sure that name is available, which means make sure nobody else is doing business with the name you want.", "context": "Importance of name availability research before business formation", "source": "Group Home Cashflow Course - Business Formation Module"}'::jsonb,
  ...
  (
    ...
    '{"quote": "Consider Ein is like your business Social Security number, so this number is gonna separate your business from your personal.", "context": "Explaining importance of federal tax ID for business operations", "source": "Group Home Cashflow Course - Business Formation Module"}'::jsonb,
...
-- Additional tactics T442-T590 will be added in subsequent batches
-- This migration file will be expanded to include all 150 tactics
...
BEGIN
  ...
  RAISE NOTICE '✓ Inserted/updated % cashflow course tactics (T439+)', inserted_count;
END $$;
...
BEGIN
  ...
  WHERE tactic_source = 'cashflow_course'
  AND tactic_id >= 'T439'
  AND (step_by_step IS NULL
       OR jsonb_typeof(step_by_step) != 'array'
       OR jsonb_array_length(step_by_step) < 3);

  IF invalid_steps > 0 THEN
    RAISE EXCEPTION '% tactics have invalid step_by_step structure', invalid_steps;
  END IF;

  RAISE NOTICE '✓ All tactics have valid JSONB structure';
END $$;
...
BEGIN
  RAISE NOTICE '
╔══════════════════════════════════════════════════════════╗
║   CASHFLOW COURSE TACTICS - PHASE 2 (PARTIAL)          ║
║   First batch inserted: T439-T441                        ║
║   Remaining batches to be added: T442-T590              ║
╚══════════════════════════════════════════════════════════╝
  ';
END $$;
