...
CREATE TABLE IF NOT EXISTS mio_knowledge_chunks (
  ...
  -- Examples: money_avoidance, impostor_syndrome, self_sabotage, comparison_catastrophe,
  -- past_prison, success_sabotage, compass_crisis, freeze_response, procrastination,
  -- overwhelm, self_doubt, motivation_collapse, performance_liability
  ...
  time_commitment_min INTEGER, -- in minutes
  time_commitment_max INTEGER, -- in minutes

  -- Difficulty level for user matching
  difficulty_level VARCHAR(20) CHECK (difficulty_level IN ('beginner', 'intermediate', 'advanced')),

  -- Temperament alignment (from Avatar Assessment)
  temperament_match TEXT[] DEFAULT '{}', -- warrior, sage, connector, builder

  -- Desired states this protocol creates
  state_created TEXT[] DEFAULT '{}', -- calm, confidence, resilience, clarity, etc.

  -- =====================================================================================
  -- Technical Metadata
  -- =====================================================================================

  tokens_approx INTEGER NOT NULL, -- Token count for context window management
  priority_level INTEGER DEFAULT 5 CHECK (priority_level BETWEEN 1 AND 10), -- 1=highest, 10=lowest
...
COMMENT ON COLUMN mio_knowledge_chunks.applicable_patterns IS
  'Identity collision patterns this protocol addresses: money_avoidance, impostor_syndrome, self_sabotage, comparison_catastrophe, past_prison, success_sabotage, compass_crisis, freeze_response, procrastination, overwhelm, self_doubt, motivation_collapse, performance_liability';

COMMENT ON COLUMN mio_knowledge_chunks.temperament_match IS
  'Avatar temperament types this protocol aligns with: warrior, sage, connector, builder';

COMMENT ON COLUMN mio_knowledge_chunks.state_created IS
  'Desired psychological states this protocol creates: calm, confidence, resilience, clarity, focus, peace, strength, etc.';
