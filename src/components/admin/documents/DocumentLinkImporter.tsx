...
interface DocumentLinkImporterProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export const DocumentLinkImporter = ({ open, onClose, onSuccess }: DocumentLinkImporterProps) => {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (parsedData.length === 0) {
        ...
        // Do not close dialog, allow user to try again
        return;
      }

      // Fetch existing documents, tactics, and links to check for validity and duplicates
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const linksToInsert = previewData.valid.map((row) => ({
        ...
        display_order: 0, // Default value, can be extended in CSV if needed
      }));
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          ...
          {isProcessing ? (
            ...
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-lg text-muted-foreground">Select a CSV file to preview links</p>
            </div>
          )}
